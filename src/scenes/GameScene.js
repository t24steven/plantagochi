...
export default class GameScene extends Phaser.Scene {
    ...
    init() {
    // Limpia la textura de la planta anterior para poder cargar una nueva
    if (this.textures.exists('plant_default')) {
        this.textures.remove('plant_default');
    }
}
    ...
    preload() {
        ...
        this.plantaData = data ? JSON.parse(data) : {
            ...
            bodyGame: '/assets/plants/plant1.png'
        };

        this.load.image('plant_default', this.plantaData.bodyGame || this.plantaData.body);

        // ── Fondo ──────────────────────────────────────────────
        this.load.image('bg', '/elements/bg/interior.png');
        ...
        this.load.image('face_normal',    '/assets/plants/face.png');

        // Efectos (blush ya está dentro de la imagen, puedes quitar esta línea)
        ...
        this.load.video('plant_die', [
            '/animation/plant_die.mp4',
            '/animation/plant_die.webm'
        ]);
    }
    ...
    setupEvents() {
        ...
        this.events.on('equip-changed', (equipped) => {
            if (this.plant) this.plant.applyEquipment(equipped);
        });
    }
    ...
    diePlant() {
        ...
        if (!isDead || this.deathVideo) return;

        this.time.removeAllEvents();
        if (this.plant) this.plant.setVisible(false);

        this.deathVideo = this.add.video(
            this.scale.width  / 2,
            this.scale.height / 2,
            'plant_die'
        ...
        this.deathVideo.on('complete', restart);
        this.deathVideo.on('error',    restart);
        this.time.delayedCall(10000, () => {
            if (this.deathVideo) restart();
        });
    }
    ...
    giveSun() {
        if (this.stats.sun < 100) {
            ...
            this.ui.showNotification('¡Le gusta el sol! ☀️');
        } else {
    ...
    changeClothes() {
        if (this.ui?.wardrobePanel) this.ui.wardrobePanel.show();
    }

    petPlant() {
        this.stats.happiness = Math.min(100, this.stats.happiness + 3);
        this.ui.showNotification('¡Le encantó! ❤️');
    }
